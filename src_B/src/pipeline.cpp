...
void pipe_check_bpred(Pipeline *p, PipelineLatch *fetch_op)
{
  if (fetch_op->trace_rec.op_type == OP_CBR)
  {
   ...
   if (pred != fetch_op->trace_rec.br_dir)
   {
    p->b_pred->stat_num_mispred++;
    fetch_op->is_mispred_cbr = true;
    p->fetch_cbr_stall = true;
   }
   if (fetch_op->trace_rec.br_dir == 0)
   {
    p->b_pred->update(fetch_op->trace_rec.inst_addr, pred, NOT_TAKEN);
   }
   else if (fetch_op->trace_rec.br_dir == 1)
   {
    p->b_pred->update(fetch_op->trace_rec.inst_addr, pred, TAKEN);
   }
  // p->b_pred->update(fetch_op->trace_rec.inst_addr, fetch_op->trace_rec.br_dir, pred);
  }
}
